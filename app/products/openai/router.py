...
async def chat_completions_endpoint(req: ChatCompletionRequest):
    ...
    try:
        ...
        if spec.is_image_edit():
            ...
            n = cfg.n if cfg is not None and cfg.n is not None else (req.n if req.n is not None else 1)
            size = cfg.size if cfg is not None and cfg.size is not None else (req.size or "1024x1024")
            response_format = (
                cfg.response_format
                if cfg is not None and cfg.response_format is not None
                else (req.response_format or "url")
            )
            ...
            vcfg = req.video_config
            from .video import validate_video_length as _validate_video_length

            seconds = vcfg.seconds if vcfg is not None and vcfg.seconds is not None else (req.seconds or 6)
            size = vcfg.size if vcfg is not None and vcfg.size is not None else (req.size or "720x1280")
            resolution_name = (
                vcfg.resolution_name
                if vcfg is not None and vcfg.resolution_name is not None
                else req.resolution_name
            )
            preset = vcfg.preset if vcfg is not None and vcfg.preset is not None else req.preset
...
async def responses_endpoint(req: ResponsesCreateRequest):
    ...
    if req.image_config is not None:
        image_options.update(req.image_config.model_dump(exclude_none=True))
    video_options = {
        key: value
        for key, value in {
            "seconds": req.seconds,
    ...
    if req.video_config is not None:
        video_options.update(req.video_config.model_dump(exclude_none=True))
