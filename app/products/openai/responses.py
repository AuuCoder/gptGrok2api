...
def _media_options(config: object | None, **fallbacks: Any) -> dict[str, Any]:
    """Merge top-level compatibility fields with an explicit media config."""
    options = {key: value for key, value in fallbacks.items() if value is not None}
    if config is None:
        return options
    if hasattr(config, "model_dump"):
        configured = config.model_dump(exclude_none=True)
...
def _media_response_output(result: MediaResult, *, model: str) -> list[dict[str, Any]]:
    if result.kind in {"image", "image_edit"}:
        output: list[dict[str, Any]] = []
        for image in result.images or []:
            encoded = str(image.get("b64_json") or "").strip()
            if not encoded:
                continue
            output.append({
                "id": make_resp_id("ig"),
        ...
        if not output:
            raise UpstreamError("Image generation returned no usable image data")
        return output
...
async def _create_media_response(
    *,
    spec: Any,
    model: str,
    ...
    """Represent Imagine output through the standard Responses envelope."""

    async def _execute() -> tuple[MediaResult, list[dict[str, Any]], str]:
        result = await run_media(
            ...
            # image_generation_call carries base64 output in the Responses API.
            image_response_format="b64_json",
        )
        output = _media_response_output(result, model=model)
        return result, output, make_resp_id("resp")

    if not stream:
        result, output, response_id = await _execute()
        output_tokens = 0
        if result.kind == "video":
            output_tokens = estimate_tokens(
                str(output[0]["content"][0].get("text") or "")
            )
        return make_resp_object(
            ...
            "completed",
            output,
            build_resp_usage(estimate_prompt_tokens(result.prompt), output_tokens),
        )
    ...
    *,
    ...
    if spec.is_image() or spec.is_image_edit() or spec.is_video():
        if tools or tool_choice is not None:
            raise UpstreamError(
                "Imagine media models do not support function tools through /v1/responses",
                status=400,
            )
        return await _create_media_response(
            spec=spec,
            model=model,
            messages=messages,
            stream=stream,
