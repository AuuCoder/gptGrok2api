...
from app.products.media import MediaResult, run_media
...
def _media_message_text(result: MediaResult) -> str:
    """Render media output in Anthropic's standard text content block.

    Anthropic Messages has no native image/video generation output block. URLs
    and the video job identifier are therefore returned as regular text, which
    keeps official Anthropic clients able to consume the response unchanged.
    """
    if result.kind in {"image", "image_edit"}:
        urls = [
            str(item.get("url") or "").strip()
            for item in (result.images or [])
            if str(item.get("url") or "").strip()
        ]
        if not urls:
            raise UpstreamError("Image generation returned no usable image URL")
        return "\n\n".join(f"![generated image]({url})" for url in urls)

    job = result.video or {}
    video_id = str(job.get("id") or "").strip()
    if not video_id:
        raise UpstreamError("Video generation did not create a job")
    return (
        "Video generation task created.\n"
        f"id: {video_id}\n"
        f"status: {str(job.get('status') or 'queued')}\n"
        f"retrieve: /v1/videos/{video_id}"
    )
...
async def _create_media_message(
    *,
    ...
    """Adapt Imagine output to the normal Anthropic Messages envelope."""

    async def _execute() -> tuple[MediaResult, str, str]:
        ...
        return result, _media_message_text(result), _make_msg_id()

    if not stream:
        result, text, msg_id = await _execute()
        input_tokens = estimate_prompt_tokens(result.prompt)
        output_tokens = estimate_tokens(text) if result.kind == "video" else 0
    ...
    async def _stream() -> AsyncGenerator[str, None]:
        ...
        text = _media_message_text(result)
...
async def create(
    *,
    ...
    # Console does not support files/audio or user tools.  Validate the raw
    # Anthropic blocks before normalisation, because unknown blocks would
    # otherwise be dropped by the generic converter.
    if spec.is_console_chat():
        from app.dataplane.reverse.protocol.xai_console_chat import validate_console_request
    ...
    if spec.is_image() or spec.is_image_edit() or spec.is_video():
        if tools or tool_choice is not None:
            raise ValidationError(
                "Imagine media models do not support function tools through /v1/messages",
                param="tools" if tools else "tool_choice",
                code="unsupported_value",
            )
        return await _create_media_message(
            spec=spec,
            model=model,
    ...
    # Console models use their own request representation.  Dispatch before
    # generic prompt/tool conversion so unsupported inputs fail explicitly.
    if spec.is_console_chat():
