...
async def lifespan(app: FastAPI):
    ...
    # Repair only accounts created by the retired Console-429 expiry policy
    # before the directory begins serving selections from its startup snapshot.
    repaired_console_429_accounts = await refresh_svc.recover_console_expired_accounts()
    if repaired_console_429_accounts:
    ...
    async def _console_reset_loop() -> None:
        while True:
            ...
            try:
                # Handles legacy ``console_429_threshold_exceeded`` records
                # whose cooldown has just elapsed before normal quota reset.
    ...
    # 7. Keep the legacy repair loop for deployments that run longer than the
    # console reset interval; it only touches the retired explicit marker.
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        ...
        description="OpenAI-compatible API gateway for GPT and Grok",
