...
async def verify_api_key(
    ...
    """Use chatgpt2api's identity store for every embedded Grok API call."""
    from api.support import require_identity

    effective_authorization = authorization
    if not effective_authorization and x_api_key:
        effective_authorization = f"Bearer {x_api_key}"
    require_identity(effective_authorization)
